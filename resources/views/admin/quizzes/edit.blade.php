...
<div class="container mx-auto px-4 py-8">
    ...
    <form action="{{ route('admin.quizzes.update', $quiz->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" id="title" class="form-input mt-1 block w-full bg-gray-100 bg-opacity-75" value="{{ $quiz->title }}" required>
        </div>
        <div class="mb-4">
            <label for="description" class="block text-gray-700 font-bold mb-2">Description</label>
            <textarea name="description" id="description" class="form-textarea mt-1 block w-full bg-gray-100 bg-opacity-75">{{ $quiz->description }}</textarea>
        </div>
        <div class="mb-4">
            <label for="time_limit" class="block text-gray-700 font-bold mb-2">Time Limit (minutes)</label>
            <input type="number" name="time_limit" id="time_limit" class="form-input mt-1 block w-full bg-gray-100 bg-opacity-75" value="{{ $quiz->time_limit }}" required>
        </div>
            ...
            @foreach($quiz->questions as $questionIndex => $question)
                <div class="question mb-4 p-4 border rounded-lg bg-gray-50 bg-opacity-75">
                    <div class="mb-4">
                        ...
                        <input type="text" name="questions[{{ $questionIndex }}][question_text]" class="form-input mt-1 block w-full bg-gray-100 bg-opacity-75" value="{{ $question->question_text }}" required>
                    </div>
                    <div class="options-container mb-4">
                        ...
                        @foreach($question->options as $optionIndex => $option)
                            <div class="option mb-2">
                                ...
                                <input type="text" name="questions[{{ $questionIndex }}][options][{{ $optionIndex }}][option_text]" class="form-input mt-1 block w-full bg-gray-100 bg-opacity-75" value="{{ $option->option_text }}" required>
                                <label for="questions[{{ $questionIndex }}][options][{{ $optionIndex }}][is_correct]" class="inline-flex items-center mt-2">
                                    <input type="checkbox" name="questions[{{ $questionIndex }}][options][{{ $optionIndex }}][is_correct]" value="1" class="form-checkbox" {{ $option->is_correct ? 'checked' : '' }}>
        ...
        <button type="submit" class="btn btn-primary bg-green-500 bg-opacity-75 text-white px-4 py-2 rounded-md hover:bg-green-600">Update Quiz</button>
    </form>
</div>
